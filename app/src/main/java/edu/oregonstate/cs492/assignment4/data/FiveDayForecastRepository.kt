...
class FiveDayForecastRepository (
    ...
    /*
     * These four properties are used to implement a basic caching strategy, where an API call
    ...
    suspend fun loadFiveDayForecast(
         ...
         */
        return if (shouldFetch(lat, lon, units)) {
            withContext(ioDispatcher) {
                try {
                    val response = service.loadFiveDayForecast(lat, lon, units, apiKey)
     ...
     *
     * @param lat The latitude for which a forecast is to be potentially fetched, as passed
     *   to `loadFiveDayForecast()`.
     * @param lon The longitude for which a forecast is to be potentially fetched, as passed
     *   to `loadFiveDayForecast()`.
     ...
     *   should be used.
     */
    private fun shouldFetch(lat: String?, lon: String?, units: String?): Boolean =
        cachedForecast == null
        || lat != currentLat
        || lon != currentLon
        || units != currentUnits
